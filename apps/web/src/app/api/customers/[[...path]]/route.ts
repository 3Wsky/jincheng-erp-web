...
/**
 * 客户 BFF 代理:携带登录 Cookie 中的令牌转发到后端 /customers/*。
 * 可选 catch-all:GET/POST /api/customers 与 /api/customers/{id}/followups 均可命中。
 */
async function proxy(
  ...
  const target = new URL(
    `${apiBaseUrl()}/customers${pathname ? `/${pathname}` : ""}`,
  );
