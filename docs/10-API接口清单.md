...
| 编号         | 方法与路径           | 作用                                                         | 当前状态 |
| ------------ | -------------------- | ------------------------------------------------------------ | -------- |
| API-TASK-001 | `GET /tasks/summary` | 我的待办：按当前用户权限聚合调拨（待审批/锁库/发出/接收）、采购（待审批/付款/收货）、盘点进行中与异常设备；由业务单据状态实时推导，无独立任务表 | VERIFY   |

登录即可访问，分组在服务端按权限过滤（出纳仅见待付款等）；审批流单据化（转办/催办/审批链）待审批矩阵签字后评估。

## 2.6 已实现的待办接口（2026-08-13）

| 编号         | 方法与路径           | 作用                                                                 | 当前状态 |
| ------------ | -------------------- | -------------------------------------------------------------------- | -------- |
| API-TASK-001 | `GET /tasks/summary` | 待办汇总：由业务单据状态实时推导（不建任务表），按当前用户权限过滤分组——调拨待审批/锁库/发出/接收（transfer:write）、采购待审批/收货（procurement:write）、采购待付款（procurement:pay）、盘点进行中（inventory:write）、异常设备待处理（inventory:read） | VERIFY   |

审批流单据化（/approvals，含转交/催办/审批链）待审批矩阵签字后设计。
