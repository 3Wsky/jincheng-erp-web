...
erDiagram
  ...
  Warehouse ||--o{ PersonalStockOrder : from_or_to
  PersonalStockOrder ||--o{ PersonalStockLine : contains
  SerialItem ||--o{ PersonalStockLine : moved_by
  Employee ||--o{ PersonalStockOrder : from_or_to
...
| `Warehouse`             | 公司、门店、个人、售后或异常库 | 仓库编码唯一；个人仓通过 `ownerEmployeeId` 归属员工（销售配权写入）；除期初导入外可由管理员创建（API-ORG-021，个人仓一人一仓）；不物理删除 |
...
| `PersonalStockOrder`    | 个人库存主单（领用/归还/转交） | 单号唯一；提交锁库、确认落位；操作人存 UserAccount.id 不建外键                 |
| `PersonalStockLine`     | 个人库存明细（一机一行）       | (orderId, serialId) 唯一；`lockedFromStatus` 供取消时原样恢复                   |
...
| `Customer`              | 客户主档（2026-08-13）         | UUID 主键；手机号不作主键、不加唯一约束（重复需可识别后受控合并）；archivedAt 软删；按组织+手机号/姓名索引 |
| `CustomerIdentity`      | 客户外部身份映射（2026-08-13） | 与 SkuExternalIdentity 同构；(sourceSystem, sourceId) 唯一；保留原始载荷与同步批次 |
| `FollowupRecord`        | 回访记录（2026-08-13）         | 实名到客户与员工；结果为 8 个标准枚举；nextFollowupAt 索引供待办到期提醒       |
...
| 业务域 | 计划实体                                                                                         | 冻结前提                       |
| ------ | ------------------------------------------------------------------------------------------------ | ------------------------------ |
| 库存   | 盘点已实现（`StocktakeOrder/Scan/Difference`，2026-08-12）；个人库存单据已实现（`PersonalStockOrder/Line`，2026-08-16）；`inventory_balance`（数量库存）、`inventory_exception`（报损/找回单据）待做 | 数量单位、负库存、报损闭环     |
...
| 客户   | 主档+回访已实现（`Customer`、`CustomerIdentity`、`FollowupRecord`，2026-08-13）；`customer_merge_log`（受控合并）待做 | 去重匹配键与合并策略待签字     |
...
- 为个人仓库补充 `ownerEmployee` 外键关系和“每名员工有效个人库唯一”约束。当前已由销售开账号写入 `ownerEmployeeId`（无 Prisma 外键，与既有约定一致）；同仓被其他员工占用时拒绝。当前已由销售开账号写入 `Warehouse.ownerEmployeeId`（无 Prisma 外键，冲突时 409），外键与唯一约束仍待补。
